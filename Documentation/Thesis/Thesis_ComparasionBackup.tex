...
In many cases, services are conceived and implemented to work together, i.e., made interoperable
by design. When systems are complex and evolve in an independent way, ensuring interoperability is
not an easy task. A typical and pragmatic solution is to resort to Web Services and XML data, sharing
schemas and namespaces, or to RESTful APIs, which are simpler to use and require that schemas
(media types) be standardized or pre-agreed. In these technologies, both customer and provider are
forced to implement full interoperability (for example, sharing a XML schema), even if only a fraction
of the possible interactions is used. This leads to a greater coupling than needed.
